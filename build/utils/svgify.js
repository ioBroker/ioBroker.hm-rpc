...
exports.toThemeSvg = toThemeSvg;
exports.svgifyAll = svgifyAll;
/**
...
function alphaToGrayPng(src) {
    ...
    return { png: pngjs_1.PNG.sync.write(out, { colorType: 0 }), width, height };
}
 ...
 */
function toThemeSvg(buffer, name) {
    const { png, width, height } = alphaToGrayPng(pngjs_1.PNG.sync.read(buffer));
...
async function svgifyAll() {
    const files = (await fs_1.default.promises.readdir(ICON_DIR)).filter(name => name.toLowerCase().endsWith('.png'));
    let written = 0;
    for (const name of files) {
        const target = path_1.default.join(ICON_DIR, name.replace(/\.png$/i, '.svg'));
        try {
            const svg = toThemeSvg(await fs_1.default.promises.readFile(path_1.default.join(ICON_DIR, name)), name);
            await fs_1.default.promises.writeFile(target, svg);
            written++;
        }
        catch (e) {
            console.warn(`  ! could not convert ${name}: ${e.message}`);
        }
    }
    console.log(`Wrapped ${written}/${files.length} icon(s) into theme-adaptive SVGs.`);
}
//# sourceMappingURL=svgify.js.map