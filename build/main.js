...
class HomematicRpc extends adapter_core_1.Adapter {
    ...
    async onReady() {
        ...
        const requiredType = this.config.daemon === 'CUxD'
            ? 'bin'
            : this.config.daemon === 'HMIP' || this.config.daemon === 'virtual-devices'
                ? 'xml'
                : undefined;
        if (requiredType && this.config.type !== requiredType) {
            this.log.warn(`${this.config.daemon} supports only ${requiredType === 'bin' ? 'BIN-RPC' : 'XML-RPC'}, the configured protocol is ignored`);
    ...
    async onMessage(obj) {
        ...
        if (obj.command === undefined ||
            ...
            obj.message === null) {
            this.log.warn(`Received invalid command via message "${obj.command}" "${JSON.stringify(obj.message)}" from ${obj.from}`);
            if (obj.callback) {
        ...
        if (obj.message.params === undefined || obj.message.params === null) {
            try {
                if (rpcClient && connected) {
                    ...
                    const data = await this.rpcMethodCallAsync(obj.command, obj.message.ID !== undefined ? [obj.message.ID, obj.message.paramType] : []);
                    if (obj.callback) {
        ...
        if (!rpcClient && !this.config.useHttps) {
            try {
                rpcClient = rpc.createClient({
                    ...
                    basic_auth: this.config.type === 'xml' && this.config.username
    ...
    async sendInit() {
        try {
            if (rpcClient && (rpcClient.connected === undefined || rpcClient.connected)) {
                this.log.debug(`${this.config.type}rpc -> ${this.config.homematicAddress}:${this.config.homematicPort}${this.homematicPath} init ${JSON.stringify([this.daemonURL, clientId])}`);
            ...
            if (message.includes('Unknown XML-RPC tag') || message.includes('Invalid XML-RPC message')) {
                ...
                this.log.error('The CCU did not answer with XML-RPC, but e.g. with an HTML page. Check the port, if the CCU requires authentication (username and password) and if the CCU accepts the HomeMatic address (e.g. use the IP address instead of a host name)');
            }
    ...
    async initRpcServer() {
                            ...
                            if (val.ADDRESS && !this.config.dontDelete) {
                                if (val.ADDRESS.includes(':')) {
                                    ...
                                    try {
                                        ...
                                        this.log.error(`Could not delete obsolete channel ${address} ${JSON.stringify(address)}: ${e.message}`);
                                    }
                                }
                                else {
                                    try {
        ...
        rpcServer.on('deleteDevices', (err, params, callback) => {
            ...
            const deviceNames = this.config.dontDelete ? [] : params[1];
            if (this.config.dontDelete) {
                this.log.info(`Devices not deleted, because "Don't delete devices" is enabled: ${JSON.stringify(params[1])}`);
            }
     ...
     */
    rpcMethodCallAsyncHelper(method, params) {
     ...
     * @param params the method specific parameters
     */
    async rpcMethodCallAsync(method, params) {
        try {
    ...
    async sendColor(channelId) {
        ...
        for (const key of ['HUE', 'SATURATION']) {
            ...
            try {
                ...
                this.log.error(`Cannot read state ${dpId}: ${e.message}`);
        ...
        try {
            ...
            this.log.error(`Cannot call putParamset: ${e.message}`);
        }
    }
    /**
    ...
    async controlEPaper(id, data) {
        ...
        tmp[3] = '3';
        tmp[4] = 'SUBMIT';
        const val = tools.combineEPaperCommand(data.lines, data.signal || '0xF0', data.tone || '0xC0', data.repeats, data.offset);
        try {
            if (rpcClient && connected) {
                await this.rpcMethodCallAsync('setValue', [`${tmp[2]}:${tmp[3]}`, tmp[4], val]);
            }
            ...
            this.log.error(`${this.config.type}rpc -> setValue ${JSON.stringify([`${tmp[2]}:${tmp[3]}`, tmp[4], val])}`);
            this.log.error(`Cannot call setValue: ${e.message}`);
        }
    }
    /**
