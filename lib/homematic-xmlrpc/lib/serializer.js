    ...
    if (current.index !== undefined) {
      ...
      switch(typeof current.value) {
        ...
        case 'object':
          ...
          // Workaround to set Double Values. Can't use CustomType because RPC Params are made on Client-Side
          // in my App
          else if (current.value.explicitDouble !== undefined) {
              appendDouble(current.value.explicitDouble, valueNode)
              stack.pop()
          }
...
function appendDouble(value, xml) {
  xml.ele('double').txt(value)
}
